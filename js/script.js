...
Sound.music.fadeOut = function() { // Fade out music function
    if(Sound.music.dom.paused == false) { // If music is playing
...
Sound.music.fadeIn = function() { // Fade in music function
    if(Sound.music.dom.paused == false) { // If music is playing
...
Sound.sfx.toggle.addEventListener('click',function() { // Toggle sfx play on click
    if(Sound.sfx.enabled) {
        Sound.sfx.dom.pause();
...
document.addEventListener('click',function() { // Toggle sound on document click (since autoplay is not allowed)
    if(Sound.documentToggled) {} else {
        Sound.music.play();
...
Cards.url = 'https://raw.githubusercontent.com/joakimskoog/AnApiOfIceAndFire/master/data/characters.json';
...
P1.num = 1;
P2.num = 2;
P1.pos = parseInt(document.getElementById('playerOne').getAttribute('data-tile'));
P2.pos = parseInt(document.getElementById('playerTwo').getAttribute('data-tile'));
Player.message = {};
...
Results.reveal = function () {
    if (Game.ended == true && Results.page.classList.contains('results--notready')) {
...
fetch(Cards.url) // Fetch the url
    ...
    .then((char) => { // Then take the parsed JSON
        ...
        // Characters name sfx
        Characters.all.eddard.sfx.name = 'sfx/names/eddard.wav';
        Characters.all.lyanna.sfx.name = 'sfx/names/lyanna.wav';
        Characters.all.balon.sfx.name = 'sfx/names/balon.wav';
        Characters.all.beric.sfx.name = 'sfx/names/beric.wav';
        Characters.all.arthur.sfx.name = 'sfx/names/arthur.wav';
        Characters.all.ramsay.sfx.name = 'sfx/names/ramsay.wav';
        Characters.all.oberyn.sfx.name = 'sfx/names/oberyn.wav';
        Characters.all.daenerys.sfx.name = 'sfx/names/daenerys.wav';
        Characters.all.robert.sfx.name = 'sfx/names/robert.wav';
        Characters.all.petyr.sfx.name = 'sfx/names/petyr.wav';

        // Quotes
        if(localStorage.getItem('daenerysQuotes')) {} else {
            localStorage.setItem('daenerysQuotes', '0');
        }
        if(localStorage.getItem('eddardQuotes')) {} else {
            localStorage.setItem('eddardQuotes', '0');
        }
        if(localStorage.getItem('lyannaQuotes')) {} else {
            localStorage.setItem('lyannaQuotes', '0');
        }
        if(localStorage.getItem('balonQuotes')) {} else {
            localStorage.setItem('balonQuotes', '0');
        }
        if(localStorage.getItem('bericQuotes')) {} else {
            localStorage.setItem('bericQuotes', '0');
        }
        if(localStorage.getItem('arthurQuotes')) {} else {
            localStorage.setItem('arthurQuotes', '0');
        }
        if(localStorage.getItem('petyrQuotes')) {} else {
            localStorage.setItem('petyrQuotes', '0');
        }
        if(localStorage.getItem('ramsayQuotes')) {} else {
            localStorage.setItem('ramsayQuotes', '0');
        }
        if(localStorage.getItem('oberynQuotes')) {} else {
        ...
        charArray = Object.values(Characters.all);
        Cards.creator(charArray, P1, 'p1');
        Cards.creator(charArray, P2, 'p2');
        console.log('%c Character objects table for debugging:','font-weight:bold;');
...
Cards.creator = function (characters, playerobject, playerid) {
    for (i = 0; i < characters.length; i++) { // Cards
        /*
        ...
        if(characters[i].sfx == '') {} else {
            playerobject.radio.setAttribute('data-sfx',characters[i].sfx.name);
        }

        if (characters[i].house == '') {} else {
            playerobject.radio.setAttribute('data-house',characters[i].house.toLowerCase());
        }

        // Load names and get names on click
        if (characters[i].Name == '') { } else { // If character name exists
            playerobject.charname.classList.add('card__name');
            playerobject.charname.innerHTML = `<span>` + characters[i].Name + `</span>`;
            ...
            playerobject.radio.addEventListener('click', function () {
                ...
                // Sound.sfx.dom.src = this.getAttribute('data-sfx');
                // Sound.sfx.dom.currentTime = 0.5;
                // Sound.sfx.play();
                Sound.music.play();
        ...
        if (characters[i].Titles.length == 1) { // If only 1 title
            playerobject.chartitles.classList.add('card__titles');
            playerobject.chartitles.innerHTML = `<span data-type="label">♔</span>` + `<span data-type="info">` + characters[i].Titles + `</span>`;
            playerobject.cardinner.appendChild(playerobject.chartitles);
        } else if (characters[i].Titles.length > 1) { // If more than 1 title
            playerobject.chartitles.classList.add('card__titles');
            playerobject.chartitles.innerHTML = `<span data-type="label">♔</span>` + `<span data-type="info">` + characters[i].Titles[Math.round(Math.random() * (characters[i].Titles.length - 1))] + `</span>`; // Get 1 random title
            playerobject.cardinner.appendChild(playerobject.chartitles);
        }
        if (characters[i].Aliases.length == 1) { // If only 1 title
            playerobject.charaliases.classList.add('card__titles');
            playerobject.charaliases.innerHTML = `<span data-type="label">♜</span>` + `<span data-type="info">` + characters[i].Aliases + `</span>`;
            playerobject.cardinner.appendChild(playerobject.charaliases);
        } else if (characters[i].Aliases.length > 1) { // If more than 1 title
            playerobject.charaliases.classList.add('card__titles');
            playerobject.charaliases.innerHTML = `<span data-type="label">♜</span>` + `<span data-type="info">` + characters[i].Aliases[Math.round(Math.random() * (characters[i].Aliases.length - 1))] + `</span>`; // Get 1 random title
            playerobject.cardinner.appendChild(playerobject.charaliases);
        }
        if (characters[i].Born == '') { } else { // If character name exists
            playerobject.charborn.classList.add('card__born');
            playerobject.charborn.innerHTML = `<span data-type="label">♥︎</span>` + `<span data-type="info">` + characters[i].Born + `</span>`;
            playerobject.cardinner.appendChild(playerobject.charborn);
        }
        if (characters[i].Died == '') { } else { // If character name exists
            playerobject.chardied.classList.add('card__died');
            playerobject.chardied.innerHTML = `<span data-type="label">✝︎</span>` + `<span data-type="info">` + characters[i].Died + `</span>`;
            playerobject.cardinner.appendChild(playerobject.chardied);
        }
        if (characters[i].Culture == '') { } else { // If character name exists
            playerobject.charculture.classList.add('card__culture');
            playerobject.charculture.innerHTML = `<span data-type="label">☗</span>` + `<span data-type="info">` + characters[i].Culture + `</span>`;
...
let changeBtnStyle = function () {
    ...
    } else if (charSelectBtn.classList.contains('characters__start--bothselected')) {
...
charSelectBtn.addEventListener('click', function () {
    if (P1.name !== '' || P2.name !== '') { // If any player picked
        if (P1.name == P2.name) { // Same picked
            Modal.sameChar();
        } else if (P1.name == '' && P2.name !== '') { // Missing p1
            Modal.missingChar('p1');
        } else if (P1.name !== '' && P2.name == '') { // Missing p2
            Modal.missingChar('p2');
        } else { // all good
            Modal.startGame();
        }
    } else if (P1.name == '' && P2.name == '') { // Both players missing
...
Modal.sameChar = function () {
    ...
    if(document.getElementById('charSelectionMessage').classList.contains('modal--2')) {
        ...
        document.getElementById('charSelectionMessage').classList.remove('modal--1');
    }
    // Add content
...
Modal.missingChar = function (missing) {
    if (missing == 'p1') { // missing p1
        ...
        // Add content
        Modal.header.innerHTML = `Will noone keep <span data-player='2'> The ` + capitalizeFirstLetter(P2.house) + `'s </span> off the throne?`;
        Modal.paragraph.innerHTML = `Please select a character for: ` + `<b data-player='1'> player one </b>`;
        Modal.choiceBtn.textContent = 'Go back';
        Modal.choiceBtn.setAttribute('data-modal-close', 'charSelectionMessage');
        Modal.choiceBtn.addEventListener('click', function (e) {
            Modal.hide('charSelectionMessage');
        ...
        // Add content
        Modal.header.innerHTML = `Will noone keep <span data-player='1'> The ` + capitalizeFirstLetter(P1.house) + `'s </span> off the throne?`;
        Modal.paragraph.innerHTML = `Please select a character for: ` + `<b data-player='2'> player two </b>`;
        Modal.choiceBtn.textContent = 'Go back';
        Modal.choiceBtn.setAttribute('data-modal-close', 'charSelectionMessage');
        Modal.choiceBtn.addEventListener('click', function (e) {
            Modal.hide('charSelectionMessage');
    ...
    } else if (missing == 'both') { // missing both
        // Create elements
        Modal.header = document.createElement('h1'); // Standard header element
        Modal.paragraph = document.createElement('p'); // Standard paragraph element
        Modal.choiceBtn = document.createElement('a'); // Standard modal button
...
Modal.startGame = function () {
    ...
    if(document.getElementById('charSelectionMessage').classList.contains('modal--both')) {
        document.getElementById('charSelectionMessage').classList.remove('modal--both');
    }
...
Instructions.showGif = function () {
    document.getElementById('instructionsImg').src = 'img/instructions.gif';
}
Instructions.dragShow = function() { // Show drag gif
    if(Instructions.dragging == false) {
        Instructions.drag.classList.add('characters__draginstruction--hover');
    }
}
Instructions.dragHide = function() { // Show drag gif
    Instructions.drag.classList.remove('characters__draginstruction--hover');
}

Instructions.dragMove = function(e) { // Move with mouse
    Instructions.drag.style.left = (e.pageX - Instructions.drag.clientWidth / 2) + 'px';
    Instructions.drag.style.top = (e.pageY - Instructions.drag.clientHeight / 2) + 'px';
}
...
Cards.sliderDrag = function (playerobject, playerid) {
    ...
    function end(e) {
        playerobject.slider.drag = false;
        Instructions.dragging = false;
        playerobject.slider.container.classList.remove('characters__slider--active');
        playerobject.slider.container.classList.remove(playerid + '__slider--active');
    ...
    if (Game.started && Game.waitTurn == false && Game.ended == false) {
        ...
        if (P1.pos < 30 && P2.pos < 30) {
            if (Dice.rolled == '1') {
                ...
                if (Player.message.isIt == 1) { // If player ones turn
                    P1.thrown1++;
                    Game.movePiece(Dice.rolled, 1, P1, false); // Move dice amount, player one, reroll is false
                } else if (Player.message.isIt == 2) { // If player twos turn
                    P2.thrown1++;
...
Game.start = function () {
    if (Game.started == false && Game.page.classList.contains('game--notstarted')) {
        ...
        if (Player.message.isIt == '1') { // Message for player One
            Player.message.content = document.createElement('h1');
            Player.message.content.innerHTML = `<span data-player='1'>` + P1.name + `</span>  goes first`; // Create label
...
Results.finish = function (winner) {
    ...
    if (Results.revealed == false) {
        ...
        if (winner == P1.name) { // If player one wins
            P1.totalWins++;
            localStorage.setItem('p1wins',P1.totalWins.toString());
        ...
        } else if (winner == P2.name) { // If player two wins
            P2.totalWins++;
            localStorage.setItem('p2wins',P2.totalWins.toString());
...
Results.revealWinner = function (winnerName, winnerObject, winnerNumber, winnerString) { // Eg. P1.name, P1, 1, 'p1'
    ...
    Results.quoteContainer.innerHTML = `<blockquote class='results__quote--text'>"` + Results.quoteText + `"</blockquote>`+`<figcaption class='results__quote--name'> - `+winnerName+`</figcaption>`;
    Results.winner.thrown1.innerHTML = `<span class='highscore__label'>Thrown 1 </span> ` + `<span class='highscore__data'>` + winnerObject.thrown1 + `</span>`;
    Results.winner.thrown2.innerHTML = `<span class='highscore__label'>Thrown 2 </span> ` + `<span class='highscore__data'>` + winnerObject.thrown2 + `</span>`;
    Results.winner.thrown3.innerHTML = `<span class='highscore__label'>Thrown 3 </span> ` + `<span class='highscore__data'>` + winnerObject.thrown3 + `</span>`;
    Results.winner.thrown4.innerHTML = `<span class='highscore__label'>Thrown 4 </span> ` + `<span class='highscore__data'>` + winnerObject.thrown4 + `</span>`;
    Results.winner.thrown5.innerHTML = `<span class='highscore__label'>Thrown 5 </span> ` + `<span class='highscore__data'>` + winnerObject.thrown5 + `</span>`;
    Results.winner.thrown6.innerHTML = `<span class='highscore__label'>Thrown 6 </span> ` + `<span class='highscore__data'>` + winnerObject.thrown6 + `</span>`;
    Results.winner.totalThrows.innerHTML = `<span class='highscore__label'>Throws: </span> ` + `<span class='highscore__data'>` + winnerObject.totalThrows + `</span>`;
    Results.winner.totalTraps.innerHTML = `<span class='highscore__label'>Traps: </span> ` + `<span class='highscore__data'>` + winnerObject.totalTraps + `</span>`;
    Results.winner.totalWins.innerHTML = `<span class='highscore__label'>Total wins </span> ` + `<span class='highscore__data'>` + winnerObject.totalWins + `</span>`;
...
Results.revealLoser = function (loserName, loserObject, loserNumber, loserString) { // Eg. P1.name, P1, 1
    ...
    // Highscore - add content
    Results.loser.thrown1.innerHTML = `<span class='highscore__label'>Thrown 1 </span> ` + `<span class='highscore__data'>` + loserObject.thrown1 + `</span>`;
    Results.loser.thrown2.innerHTML = `<span class='highscore__label'>Thrown 2 </span> ` + `<span class='highscore__data'>` + loserObject.thrown2 + `</span>`;
    Results.loser.thrown3.innerHTML = `<span class='highscore__label'>Thrown 3 </span> ` + `<span class='highscore__data'>` + loserObject.thrown3 + `</span>`;
    Results.loser.thrown4.innerHTML = `<span class='highscore__label'>Thrown 4 </span> ` + `<span class='highscore__data'>` + loserObject.thrown4 + `</span>`;
    Results.loser.thrown5.innerHTML = `<span class='highscore__label'>Thrown 5 </span> ` + `<span class='highscore__data'>` + loserObject.thrown5 + `</span>`;
    Results.loser.thrown6.innerHTML = `<span class='highscore__label'>Thrown 6 </span> ` + `<span class='highscore__data'>` + loserObject.thrown6 + `</span>`;
    Results.loser.totalThrows.innerHTML = `<span class='highscore__label'>Throws: </span> ` + `<span class='highscore__data'>` + loserObject.totalThrows + `</span>`;
    Results.loser.totalTraps.innerHTML = `<span class='highscore__label'>Traps: </span> ` + `<span class='highscore__data'>` + loserObject.totalTraps + `</span>`;
    Results.loser.totalWins.innerHTML = `<span class='highscore__label'>Total wins </span> ` + `<span class='highscore__data'>` + loserObject.totalWins + `</span>`;
...
Characters.getQuote = function(charobject,storageid) { // Eg: Characters.all.daenerys.quotes,'daenerysQuotes'
    Results.quoteNumber = parseInt(localStorage.getItem(storageid));
    if(Results.quoteNumber < charobject.length) { // If within quote array
...
Game.trapCheck = function(reroll, player, playerobject) {
    Game.waitTurn = true;
    if(Game.trapsActive) {
        if(playerobject.pos == Game.traps.dayne.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.dayne); // Run trap
        } else if(playerobject.pos == Game.traps.martell.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.martell); // Run trap
        } else if(playerobject.pos == Game.traps.dondarrion.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.dondarrion); // Run trap
        } else if(playerobject.pos == Game.traps.baratheon.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.baratheon); // Run trap
        } else if(playerobject.pos == Game.traps.targaryen.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.targaryen); // Run trap
        } else if(playerobject.pos == Game.traps.mormont.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.mormont); // Run trap
        } else if(playerobject.pos == Game.traps.greyjoy.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.greyjoy); // Run trap
        } else if(playerobject.pos == Game.traps.baelish.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.baelish); // Run trap
        } else if(playerobject.pos == Game.traps.bolton.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.bolton); // Run trap
        } else if(playerobject.pos == Game.traps.stark.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.stark); // Run trap
        } else if(playerobject.pos == Game.traps.nightking.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.nightking); // Run trap
        } else if(playerobject.pos == Game.traps.melisandre.pos) { // If trap hit
            Game.trapRun(player, playerobject, Game.traps.melisandre); // Run trap
        } else { // If trap not hitelse {
...
Game.trapRun = function(player, playerobject,trap) {
    playerobject.totalTraps++; // Increase total traps hi
    Game.page.classList.add('game'+trap.modclass); // Add styling class
    Game.messageContainer.classList.add('game__turn--show');
    document.getElementById('sfxMenu').classList.add('sound--hidden');
    if(trap.friendlyTo == playerobject.name || Game.trapsAreAllFriendly == true) { // If friendly
    ...
    setTimeout(function() {
        ...
        setTimeout(function () { // Move piece after message delay
            ...
            if(playerobject.pos <= 0) { // If less than tile 1
                playerobject.token.setAttribute('data-tile', 1); // Reset to 1
                playerobject.pos = parseInt(playerobject.token.getAttribute('data-tile'));
            } else if (playerobject.pos >= 30) {
                if(playerobject.pos > 30) {
        ...
        if(player == 1) {
            ...
            if (reroll == true) {
                ...
                setTimeout(function () { // Delay to give time to view dice roll and message
                    Player.message.content.innerHTML = `<span data-player='1'>` + P1.name + `</span> gets to reroll!`; // Reroll player message
...
Difficulty.value.innerHTML = 'Normal<br>- Beware of traps';
Difficulty.range.oninput = function() {
    if (Difficulty.range.value == 1) {
        Difficulty.value.innerHTML = 'Childlike<br>- All traps become your playmates.';
        Difficulty.adjust('childlike');
    } else if (Difficulty.range.value == 2) {
        Difficulty.value.innerHTML = 'Normal<br>- Beware of traps';
        Difficulty.adjust('normal');
    } else if (Difficulty.range.value == 3) {
        Difficulty.value.innerHTML = 'Hard<br>- Traps are given double espresso.<br>- Boosts take a sleeping pill.';
        Difficulty.adjust('hard');
    } else if (Difficulty.range.value == 4) {
        Difficulty.value.innerHTML = 'Insane.<br>- All traps are the <span data-trap="nightking">Nightking</span>,<br> on steroids.';
        Difficulty.adjust('insane');
    }
}
//
// PLEASE EXCUSE THIS REPETITIVE CLUTTER OF CODE
Difficulty.adjust = function(value) {
    if(Game.trapsAreAllFriendly == true) { // Reset after changing from childlike difficulty
        Game.trapsAreAllFriendly = false;
    }
    if(value == 'childlike') {
        Game.trapsAreAllFriendly = true;
        Game.traps.dayne.penaltyTo = -1;
        Game.traps.dayne.friendlyBoost = 4;

        Game.traps.martell.penaltyTo = -3;
        Game.traps.martell.friendlyBoost = 3;

        Game.traps.dondarrion.penaltyTo = -1;
        Game.traps.dondarrion.friendlyBoost = 2;

        Game.traps.baratheon.penaltyTo = -1;
        Game.traps.baratheon.friendlyBoost = 1;

        Game.traps.targaryen.penaltyTo = -2;
        Game.traps.targaryen.friendlyBoost = 1;

        Game.traps.mormont.penaltyTo = -4;
        Game.traps.mormont.friendlyBoost = 2;

        Game.traps.greyjoy.penaltyTo = -5;
        Game.traps.greyjoy.friendlyBoost = 4;

        Game.traps.baelish.penaltyTo = -4;
        Game.traps.baelish.friendlyBoost = 2;

        Game.traps.bolton.penaltyTo = -4;
        Game.traps.bolton.friendlyBoost = 3;

        Game.traps.stark.penaltyTo = -1;
        Game.traps.stark.friendlyBoost = 3;

        Game.traps.nightking.penaltyTo = -20;

        Game.traps.melisandre.penaltyFrom = 18;
        Game.traps.melisandre.penaltyTo = 7;
