...
Modal.append = function(container,element) { // Append function for standard elements
...
for(i=0; i < Modal.closeBtn.length; i++) {
    ...
    for(x=0;x<modalIdArray.length;x++) {
        if(document.getElementById(modalIdArray[x]).classList.contains('modal--open')) {
            document.getElementById(modalIdArray[x]).classList.add('modal--closed');
            document.getElementById(modalIdArray[x]).classList.remove('modal--open');
            document.getElementById(modalIdArray[x]).classList.remove('modal--open');
        }
    }
